...
import { HOME_HERO_QUERY } from '../src/sanity/queries';
import type { Hero } from '../src/sanity/types';

const STEP_ICONS = [Palette, Users, ShieldCheck];
        ...
        <div ref={heroRef} className="relative min-h-[90vh] flex items-center justify-center overflow-hidden px-6 pt-20">
            ...
            <div className="max-w-5xl text-center space-y-8">
                ...
                <h1 className="hero-title text-6xl md:text-9xl font-display font-bold tracking-tighter leading-none text-white">
                    {hero?.headlineLine1}<br /> <span className="text-[#74C63D]">{hero?.headlineLine2}</span>
                </h1>
                {hero?.subtitle ? (
...
const HowItWorks = ({ hero }: { hero: Hero | null }) => {
        ...
        <section ref={sectionRef} className="py-24 px-6 bg-[#0B0F0B]">
            <div className="max-w-7xl mx-auto">
                <div className="text-center mb-16 space-y-4">
                    <h2 className="text-4xl md:text-6xl font-display font-bold tracking-tight">{hero?.howItWorksTitle}</h2>
                    {hero?.howItWorksSubtitle ? (
                        <p className="text-[#7C857C] text-lg">{hero.howItWorksSubtitle}</p>
                    ) : null}
                </div>
                {steps.length > 0 && (
...
export const Home = () => {
    const { data: hero, loading } = useSanityData<Hero>(HOME_HERO_QUERY);
    ...
    const hasTeaser =
        hero?.teaserLine1 ||
        hero?.teaserLine2 ||
        hero?.teaserDescription ||
        hero?.teaserImageUrl;

    return (
        <div className="space-y-0">
            <Hero hero={hero} />
            <HowItWorks hero={hero} />
                    ...
                    <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-20 items-center">
                        <div ref={teaserTitleRef} className="space-y-8">
                            <h2 className="text-5xl md:text-7xl font-display font-bold leading-none">
                                {hero?.teaserLine1}<br /><span className="text-[#74C63D]">{hero?.teaserLine2}</span>
                            </h2>
                            {hero?.teaserDescription ? (
                            ...
                            {(hero?.stat1Value || hero?.stat2Value) ? (
                                <div className="flex gap-8 border-l-2 border-[#1C261C] pl-8">
                                    {hero?.stat1Value ? (
                                        <div>
                                            <p className="text-3xl font-display font-bold text-[#74C63D]">{hero.stat1Value}</p>
                                            <p className="text-sm text-[#7C857C]">{hero.stat1Label}</p>
                                        </div>
                                    ) : null}
                                    {hero?.stat2Value ? (
                                        <div>
                                            <p className="text-3xl font-display font-bold text-[#74C63D]">{hero.stat2Value}</p>
                                            <p className="text-sm text-[#7C857C]">{hero.stat2Label}</p>
                                        </div>
                                    ) : null}
                                </div>
                            ) : null}
            ...
            {hero?.ctaTitle ? (
                <section className="py-24 px-6 bg-[#74C63D]">
                    <div className="max-w-4xl mx-auto text-center space-y-8">
                        <h2 className="text-4xl md:text-6xl font-display font-bold text-black tracking-tighter">{hero.ctaTitle}</h2>
                        {hero?.ctaButtonText ? (
                            <a
